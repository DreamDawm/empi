...
当患者身份证号为空或无效时，系统采用姓名匹配合并策略：

1. **优先匹配 empi_master**：在已建立主索引的患者中查找姓名相同的记录
2. **姓名匹配维度**：
   - 汉字姓名精确匹配
   - 拼音姓名匹配
3. **合并条件**：除身份证外，超过一半的字段得满分（100分）

### 14.2 决策类型

| 类型 | 条件 | 说明 |
|------|------|------|
| DIRECT_MERGE | 身份证+姓名完全相同 | 最高优先级 |
| NAME_MATCH_MERGE | 无有效身份证，姓名匹配且>50%字段满分 | 次优先级 |
| AUTO_MERGE | 相似度≥阈值 | 常规自动合并 |
| PENDING_REVIEW | 相似度<阈值 | 人工审核 |